...
### ⚠️ 다른 덱에도 해당하는 함정 — step-box 가 펼쳐지지 않는다

공유 CSS의 step-box는 `max-height: 0 → 600px` **전이**로 펼치는데, 이 전이가 Reveal 의
슬라이드 `display` 토글과 겹치면 **`.visible` 을 붙여도 계산 스타일이 갱신되지 않는** 상태가 생깁니다.
클래스도 CSS도 맞는데 `getComputedStyle` 이 `max-height: 0px`, `opacity: 0` 을 돌려줍니다.
(같은 자리에 새로 만든 요소는 정상, 원본을 DOM에서 뗐다 붙이면 복구 — 즉 요소 하나의 재계산 누락)

이 덱은 전이를 버리고 `display: none/block` 으로 바꿨습니다. 표시 조건은 규약대로 `.visible` 하나뿐입니다.
**다른 덱(1-2-3차시 등 step-box 19개)도 수업 전에 실제로 눌러서 확인해 보시는 것이 좋겠습니다.**

여백 명시도도 함께 걸렸습니다 — `section.tight .step-box`(0,2,1)는 공유 CSS의
`.reveal .step-box.visible`(0,3,0)에 집니다. **여백은 `.visible` 쪽에 둬야 이깁니다.**

**검증 방법 교훈** — `classList.add('visible')` 로 강제 전개해 재면 이 버그가 안 잡힙니다.
강제로 붙인 클래스도 똑같이 무시되어 931px 로 나왔고, 실제 클릭으로 재니 1023px 이었습니다.
**인터랙션은 실제로 눌러서 재야 합니다.**
